  ...
  <div class="workoutlog-wrapper">
    ...
    <form v-else class="workoutlog-form" @submit.prevent="submitForm">
      ...
      <b-field label="Peso (kg)"
        ...
        <b-input type="number" v-model.number="workoutLogData.weight" min="0"></b-input>
      </b-field>
...
import z from 'zod'
import { getWorkoutlog, updateWorkoutlog } from '../../api/workoutlog'
...
const workoutLogSchema = z.object({
  ...
  weight: z.number().min(0, "Peso no puede ser negativo"),
...
const workoutLogError = ref<WorkoutLogError>({
  exerciseValid: [], setsValid: [], repsValid: [], weightValid: [], dateValid: [], server: []
})

const disabledButton = computed(() => !workoutLogSchema.safeParse(workoutLogData.value).success)

async function fetchWorkoutlog() {
  const { success, data, errorCode } = await getWorkoutlog(id)
  if (success) {
    workoutLogData.value = {
...
async function fetchExercises() {
  ...
  else if (errorCode === 401) router.push('/unauthorized')
  else router.push('/not-found')
}
...
async function submitForm() {
  ...
  const { success, error } = await updateWorkoutlog(id, workoutLogData.value)
